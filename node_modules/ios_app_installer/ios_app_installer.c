...
int app_exists(AMDeviceRef device)
{
    ...
    assert(AMDeviceStopSession(device) == 0);
    assert(AMDeviceDisconnect(device) == 0);
    if (appExists) {printf("the app %s exists\n", bundle_id);}
    else {printf("couldn't find app %s\n", bundle_id);}
    return appExists;
}
...
void handle_device(AMDeviceRef device) {
    ...
    if (install) {printf("install = true\n");}
    if (uninstall) {printf("uninstall = true\n");}
    if (command_only) {printf("command: %s\n", command);}
